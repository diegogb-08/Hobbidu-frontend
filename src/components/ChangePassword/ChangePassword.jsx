...
const ChangePassword = (props) => {
  ...
  const handleChange = (e) => {
    ...
    setMessage('')
    if (Object.keys(errors).length > 0) { setErrors(validate({ ...password, [e.target.name]: e.target.value, [e.target.name]: e.target.value }, 'register')) }
  }
  ...
  const toggle = async () => {
    ...
    try {
      const result = await axios.put(port + customer + '/change_password/' + props.user._id, body, auth)
      if (result) {
